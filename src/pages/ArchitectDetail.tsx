  ...
  useEffect(() => {
    const fetchArchitect = async () => {
      try {
        ...
        toast({
          title: "Error",
          description: "Failed to load architect",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const handleWhatsApp = async () => {
    try {
      ...
      const message = encodeURIComponent(
        `Hi, I'm interested in your architecture service: ${architect?.title}`
      );
  ...
  if (!architect) {
    return (
      <div className="min-h-screen bg-background">
        <Navbar />
        <div className="container mx-auto px-4 py-12">
          <p className="text-muted-foreground">Arkitekti nuk u gjet.</p>
        </div>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <Link to="/architects" className="mb-6 flex items-center gap-2 text-accent hover:text-accent/80">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div>
            <Badge className="mb-4">{architect.specialty || "Arkitekturë"}</Badge>
            <h1 className="mb-2 text-4xl font-bold text-foreground">{architect.title}</h1>
            <div className="mb-4 flex items-center gap-4">
              <div className="flex items-center gap-1 text-lg">
                <Star className="h-5 w-5 fill-accent text-accent" />
                <span>{architect.rating || 0}</span>
              </div>
              <div className="flex items-center gap-1 text-lg">
                <Briefcase className="h-5 w-5 text-muted-foreground" />
                <span>{architect.projects || 0} projekte</span>
              </div>
            </div>

            <div className="mb-6 flex items-center gap-2 text-lg text-muted-foreground">
              <MapPin className="h-5 w-5" /> {architect.location}
            </div>

            <p className="mb-8 text-lg text-foreground">{architect.description}</p>

            <div className="mb-8 rounded-lg bg-muted p-6">
              <p className="mb-2 text-sm font-medium text-muted-foreground">Çmimi</p>
              <p className="text-3xl font-bold text-primary">€{architect.price.toLocaleString()}</p>
            </div>

            <div className="mb-6 space-y-4">
              <p className="text-sm font-medium text-muted-foreground">Kontakti</p>
              <p className="text-foreground">{architect.userId?.fullName}</p>
              <p className="text-foreground">{architect.userId?.companyName}</p>
            </div>
